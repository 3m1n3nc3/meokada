...
class User extends Generic{
    ...
    public function verifyProLevel($user_id = null, $set_community = 0) {
        ...
        $user_community = self::listCommunityPlans($user['community']);
        $ver_community  = self::listCommunityPlans($set_community);

        $user_community_price = !empty($user_community['price']) ? $user_community['price'] : 0;
        $ver_community_price  = !empty($ver_community['price']) ? $ver_community['price'] : 0; 

        return array(
            'pro_level' => $pro_level, 
            'community' => $user['community'],  
            'community_access' => ($user_community_price > 0),
        ); 
    }
	...
	public static function registerUser(){
		...
		if( $email_validation == "on" ){
            if (!isset($_SESSION['email_code'])) {
                $e_code = sha1(time() + rand(111,999));
                $_SESSION['email_code'] = $e_code;
            }
